...
resource "aws_api_gateway_deployment" "text_narrator_deployment" {
  rest_api_id = aws_api_gateway_rest_api.text_narrator_api.id
  depends_on  = [aws_api_gateway_integration.text_narrator_integration] # ensures integration is ready
}

resource "aws_api_gateway_stage" "text_narrator_stage" {
  rest_api_id   = aws_api_gateway_rest_api.text_narrator_api.id
  deployment_id = aws_api_gateway_deployment.text_narrator_deployment.id
  stage_name    = "prod"
}
...
resource "aws_s3_bucket_policy" "frontend_bucket_policy" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Allow whoever runs Terraform to upload/list
      {
        Sid       = "AllowUserUpload"
        Effect    = "Allow"
        Principal = {
